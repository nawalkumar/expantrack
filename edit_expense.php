...
<div class="bg-white p-5 mt-5 shadow-lg rounded-lg w-11/12 md:w-7/10 lg:w-1/2 mx-auto mb-14 break-words hover-grow transition-transform duration-300 ease-in-out rounded-lg">
    <h1 class="ml-5 mb-12 text-4xl font-bold">Edit Expense</h1>
    <form action="edit_expense.php?id=<?php echo $expense_id; ?>" method="POST">
        <input type="text" name="item_name" placeholder="Item Name" value="<?php echo htmlspecialchars($expense['item_name']); ?>" required class="block w-11/12 mb-4 ml-6 p-2 border border-gray-300 rounded-lg">
        <input type="number" name="amount" placeholder="Amount" value="<?php echo $expense['amount']; ?>" required min= "0" class="block w-11/12 mb-4 ml-6 p-2 border border-gray-300 rounded-lg">
        <input type="date" name="date" placeholder="Date" value="<?php echo $expense['date']; ?>" required class="block w-11/12 mb-4 ml-6 p-2 border border-gray-300 rounded-lg">
        <textarea name="description" placeholder="Description" required class="block w-11/12 mb-4 ml-6 p-2 border border-gray-300 rounded-lg"><?php echo htmlspecialchars($expense['description']); ?></textarea>
